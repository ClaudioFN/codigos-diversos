...
def create_whole_program():
    # Creates a instance of the window of the program
    window_instance = create_program_window()
    
    # Create the association with the logo for the program
    pw.create_window_logo(window_instance, config_definitions.PROGRAM_LOGO)

    # Radio Button selection for the place to save the data
    v = tk.StringVar(window_instance, config_definitions.SAVE_DATA)
    ...
    radio_button_db  = tk.Radiobutton(window_instance, text="DB", variable=v, value="DB", image=db_img)
    radio_button_csv = tk.Radiobutton(window_instance, text="CSV", variable=v, value="CSV", image=csv_img)
    ...
    for i, labels_availables in enumerate(fields_names):
        program_labels.program_labels_definition(window_instance, labels_availables, i + 1)

    #Enter fields
    tk.Label(window_instance, text=f"{config_definitions.SAVE_LOCATION_TEXT}:").grid(row=0, column=0, sticky="w")

    entry_cpf_cnpj = tk.Entry(window_instance, width=30)
    ...
    label_cpf_cnpj = tk.Label(window_instance, text="-")
    label_cpf_cnpj.grid(row=1, column=2)

    entry_name = tk.Entry(window_instance, width=30)
    ...
    label_name = tk.Label(window_instance, text="-")
    label_name.grid(row=2, column=2)

    entry_address = tk.Entry(window_instance, width=30)
    ...
    label_address = tk.Label(window_instance, text="-")
    label_address.grid(row=3, column=2)

    entry_neighborhood = tk.Entry(window_instance, width=30)
    ...
    label_neighborhood = tk.Label(window_instance, text="-")
    label_neighborhood.grid(row=4, column=2)

    entry_city = tk.Entry(window_instance, width=30)
    ...
    label_city = tk.Label(window_instance, text="-")
    label_city.grid(row=5, column=2)

    entry_state = tk.Entry(window_instance, width=30)
    ...
    label_state = tk.Label(window_instance, text="-")
    label_state.grid(row=6, column=2)

    entry_fu = tk.Entry(window_instance, width=30)
    ...
    label_fu = tk.Label(window_instance, text="-")
    label_fu.grid(row=7, column=2)

    entry_main_phone = tk.Entry(window_instance, width=30)
    ...
    entry_mobile_phone = tk.Entry(window_instance, width=30)
    ...
    label_phones = tk.Label(window_instance, text="-")
    label_phones.grid(row=10, column=1, columnspan= 20)
    ...
    # Register Button
    button = tk.Button(window_instance, text="Register", command = lambda: execution(window_instance))
...
if __name__ == '__main__':
    print('\nCreating the window to interact with the program\n')
    create_whole_program()